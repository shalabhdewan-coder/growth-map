...
- [x] Source from NCERT EVS/Science Class 1-6 + Singapore MOE Science syllabus + NSO/iOS sample papers per the rung topics already named in the array (`Living & non-living`, `Plants`, `Animals & classification`, … through `Scientific method & design`). **Sarah's Science ladder complete 2026-07-20** (rungs 1-8 done earlier session, rungs 9-12 this session) — all 12 rungs now have 3 `sc` + 3 `method` entries each. NCERT Class 4 EVS confirmed too narrative/integrated for direct citation (chapter-title scan done for rungs 9-11, no overlap found); NCERT Class 6/7 Science remain unusable stub PDFs (unchanged from earlier finding). **Mirayah's Science ladder complete 2026-07-20** (rungs 1-5 done earlier session, rungs 6-10 this session — the LAST task of the entire 4-task curriculum upgrade project) — all 10 rungs now have 3 `sc` + 3 `method` entries each. Rung 6 "Hot & cold" and rung 8 "Float or sink" grounded in genuine MOE Lower Block outcomes (heat/water-state changes; float/sink as a named material property); rung 7 "Push & pull" strongly grounded in MOE's literal "Interaction of Forces (Magnets)" Lower Block outcome; rung 9 "Day & night" confirmed to have zero MOE/SOF coverage at any band (grepped, honest general-knowledge as anticipated); rung 10 "Little scientist" grounded in the *observe/predict* half of MOE's cross-cutting "Skills and Processes" section (p.6-7) — deliberately kept below Sarah's rung 12 (hypothesis/variables), matching MOE's own skill sequencing.
- [x] Olympiad-woven rungs (per your "weave into core" decision): upper rungs' `sc` (think-it-through) and `method` (like-a-scientist) entries should read closer to NSO/iOS question style, not just curriculum-recall. Rungs 9-10 grounded in genuine MOE Upper-Block content (Human System integration, Cell System plant-vs-animal parts); rung 11 grounded in one previously-unused SOF Class 5 item (burning-coal T/F) plus honest general-knowledge (MOE has zero dissolving/chemistry content, re-confirmed by grep); rung 12 grounded in MOE's own "Skills and Processes" section (Investigation/hypothesis/fair-methods, p.6-7) plus SOF Achievers-Section HOTS reasoning *style* (no literal fair-test/IV-DV question exists in the SOF Class 1/3/5 samples — confirmed by grep, so this rung's rigor is constructed in that style, not lifted from a specific paper, and that distinction is stated in `content/science-sarah-rung12.md`).
- [x] Verification protocol per rung — `node --check` pass; direct `ws_science_sarah(li)` calls, 6 runs × all 12 rungs (72 total), zero `undefined`/`NaN`/`[object Object]` hits, block/answer structure consistent across every run. Mirayah rungs 6-10: `node --check` pass; direct `ws_science_mirayah(li)` calls, 6 runs × 5 rungs (30 total), zero bad-content hits. **Closing full-project sweep (2026-07-20):** both science ladders together — Sarah 12 rungs × 5 + Mirayah 10 rungs × 5 = 110 total calls, zero `undefined`/`NaN`/`[object Object]` hits. Ladder-extension check: pushed both science ladders 3 levels past their hand-authored ceiling via `intensifyRung`/`BASE_LADDER_LEN` and called the worksheet generator on each synthetic rung — zero crashes (content does not escalate past ceiling since science rungs are fixed content, not knob-scaled — same documented limitation as the maths ladder-extension work, confirmed here as "no crash," not "escalates").
