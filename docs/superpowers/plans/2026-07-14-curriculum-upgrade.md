...
- [x] Verification protocol per rung — `node --check` pass; direct `ws_science_sarah(li)` calls, 6 runs × all 12 rungs (72 total), zero `undefined`/`NaN`/`[object Object]` hits, block/answer structure consistent across every run.
